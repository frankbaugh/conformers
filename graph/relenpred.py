...
def graph2data(pair):
    ...
    data = Data(
        x0 = conf0.x,
        edge_index0 = conf0.edge_index,
        edge_attr0 = conf0.edge_attr,
        x1 = conf1.x,
        edge_index1 = conf1.edge_index,
        edge_attr1 = conf1.edge_attr,
        y = pair[2]
        )

def read_csv():
    "Returns a dictionary of conf names and their energies relative to CCSD(T) calculations"
    energies = {}
    filename = path + 'rel_en.csv'
    data = np.genfromtxt(filename, delimiter=',', dtype=None, encoding=None)
    for row in data:
        conf_name = row[0] + "_" + str(row[1])
        energies[conf_name] = row[2]
    

energies = read_csv()

def energy_comparison(conf1, conf2):
    
    en1 = energies[conf1]
    en2 = energies[conf2]
    
    return torch.Tensor([en1 - en2])


def pair_data(data_list):
    ...
    for conf_i in range(nconfs):
        for conf_j in range(conf_i +1, nconfs):
            ...
            paired_data_list.append(graph2data(pair1))
            
            pair2 = [data_list[mol + conf_j], data_list[mol + conf_i], label2]
            paired_data_list.append(graph2data(pair2))
...
paired_data = pair_data(data_list)
