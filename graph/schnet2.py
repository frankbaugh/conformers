...
class CombinedNetwork(nn.Module):
    ...
    def forward(self, z, pos, batch):
        ...
        y1 = self.schnet1(z[:, 0, ...], pos[:, 0, ...], batch)
        y2 = self.schnet2(z[:, 1, ...], pos[:, 1, ...], batch)
...
def train_loop(dataloader, model, loss_fn, optimiser, train_loss_list):
    ...
    for batch, z, pos, labels in enumerate(train_loaded):
        print(data)
...
def test_loop(dataloader, model, loss_fn, test_loss_list):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            correct += ((pred > 0.5).squeeze().long() == data.labels.squeeze().long()).sum().item()
